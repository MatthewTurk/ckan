...
ENV HOME=/root DEBIAN_FRONTEND=noninteractive \
  CKAN_HOME=/usr/lib/ckan/default CKAN_CONFIG=/etc/ckan/default CKAN_DATA=/var/lib/ckan

# Install required packages
RUN apt-get -qy update && \
  apt-get -qy install python-minimal python-dev python-virtualenv libevent-dev \
    libpq-dev nginx-light apache2 libapache2-mod-wsgi postfix build-essential && \
  apt-get clean && rm -rf /var/lib/apt/lists/* /tmp/* /var/tmp/*

# Install CKAN
RUN virtualenv $CKAN_HOME
RUN mkdir -p $CKAN_HOME $CKAN_CONFIG $CKAN_DATA /var/cache/nginx
...
RUN a2ensite ckan_default && a2dissite 000-default
